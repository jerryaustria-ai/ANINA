...
const tierSchema = new mongoose.Schema(
  {
    ...
    eligibleClassIds: [{ type: mongoose.Schema.Types.ObjectId, ref: "ClassDefinition" }],
...
tierSchema.methods.toPublic = function () {
  return {
    ...
    eligibleClassIds: (this.eligibleClassIds || []).map((item) => item?._id || item),
    eligibleClasses: (this.eligibleClassIds || []).map((item) =>
